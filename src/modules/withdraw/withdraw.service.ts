...
export class WithdrawService {
    ...
    async createRequest(userId: string, dto: CreateWithdrawRequestDto) {
        ...
        if (user.role !== UserRole.VENDOR && user.role !== UserRole.DRIVER && user.role !== UserRole.MANAGER) {
    ...
    async completeRequest(id: string, dto: AdminCompleteWithdrawDto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (
                ...
                this.logger.warn(`Complete request failed: Request ${id} has invalid status ${request.status}`);
            ...
            if (user.role === UserRole.DRIVER) {
                const driverProfile = await tx.driverProfile.findUnique({ where: { userId: request.userId } });
                if (!driverProfile) throw new BadRequestException('DRIVER_PROFILE_NOT_FOUND');

                if (driverProfile.walletAmount.toNumber() < amount) {
                    this.logger.warn(`Withdraw failed for Driver ${request.userId}: Insufficient funds. Balance: ${driverProfile.walletAmount}, Request: ${amount}`);
                    throw new BadRequestException('INSUFFICIENT_FUNDS');
                }

                await this.walletService.updateDriverWallet(request.userId, amount, WalletConstants.OPERATION_SUBTRACT, tx);

            } else if (user.role === UserRole.VENDOR && user.vendorId) {
                const vendor = await tx.vendor.findUnique({ where: { id: user.vendorId } });
                if (!vendor) throw new BadRequestException('VENDOR_PROFILE_NOT_FOUND');

                if (vendor.walletAmount.toNumber() < amount) {
                    this.logger.warn(`Withdraw failed for Vendor ${user.vendorId}: Insufficient funds. Balance: ${vendor.walletAmount}, Request: ${amount}`);
                    throw new BadRequestException('INSUFFICIENT_FUNDS');
                }

                await this.walletService.updateVendorWallet(user.vendorId, amount, WalletConstants.OPERATION_SUBTRACT, tx);

            } else {
                // Fallback: Customer Profile (Used by Customers, Managers, etc.)
                const customerProfile = await tx.customerProfile.findUnique({ where: { userId: request.userId } });
                if (!customerProfile) {
                    ...
                    this.logger.error(`Withdraw failed: No wallet profile found for user ${request.userId} with role ${user.role}`);
                    throw new BadRequestException('WALLET_PROFILE_NOT_FOUND');
                }

                if (customerProfile.walletAmount.toNumber() < amount) {
                    this.logger.warn(`Withdraw failed for User ${request.userId}: Insufficient funds.`);
                    throw new BadRequestException('INSUFFICIENT_FUNDS');
                }

                await this.walletService.updateUserWallet(request.userId, amount, WalletConstants.OPERATION_SUBTRACT, tx);
            }
            ...
            this.logger.log(`Withdraw request ${id} completed for user ${request.userId}. Amount: ${amount}`);
