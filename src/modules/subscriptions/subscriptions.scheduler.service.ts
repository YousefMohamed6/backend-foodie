          ...
          if (vendor.subscription && vendor.subscriptionId) {
            await tx.subscriptionEventLog.create({
              data: {
                ...
                planName: vendor.subscriptionPlan?.englishName || vendor.subscriptionPlan?.arabicName || 'Expired Plan',
        ...
        if (daysRemaining >= 1 && daysRemaining <= 3) {
          try {
            ...
            const titleEn = await this.i18n.translate(
              'messages.SUBSCRIPTION_EXPIRING_TITLE',
              { lang: 'en' },
            );
            ...
            const bodyEn = await this.i18n.translate(
              'messages.SUBSCRIPTION_EXPIRING_BODY',
              {
                lang: 'en',
            ...
            this.logger.log(
              `Sent expiry notification to vendor ${vendor.id} (${daysRemaining} days left)`,
            );
          } catch (notifError) {
            this.logger.error(
              `Failed to send expiry notification to vendor ${vendor.id}:`,
              notifError,
            );
          }
        }
