...
export class AppleAuthService {
    ...
    constructor(private configService: ConfigService) {
        ...
        if (!this.appleClientId) {
            this.logger.warn('APPLE_CLIENT_ID not configured. Apple Sign-In will not work.');
        }
    }
    ...
    async verifyIdentityToken(identityToken: string): Promise<AppleTokenPayload> {
        try {
            ...
            if (!payload || !payload.sub || !payload.email) {
                throw new UnauthorizedException('Invalid Apple token: missing required fields');
            }

            this.logger.log(`Successfully verified Apple token for user: ${payload.email}`);
            ...
            this.logger.error(`Apple token verification failed: ${(error as Error).message}`);
