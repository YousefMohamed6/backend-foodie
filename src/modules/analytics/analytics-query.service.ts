...
export class AnalyticsQueryService {
            ...
            performance: snapshot
                ? {
                    acceptanceRating: getPerformanceRating(
                        ...
                        {
                            excellent: AnalyticsConfig.PERFORMANCE_THRESHOLDS.VENDOR_ACCEPTANCE_RATE_EXCELLENT,
                            good: AnalyticsConfig.PERFORMANCE_THRESHOLDS.VENDOR_ACCEPTANCE_RATE_GOOD,
                        },
                        true,
                    ),
                    revenueRating: Number(snapshot.totalRevenue) > 10000 ? 'EXCELLENT' : Number(snapshot.totalRevenue) > 5000 ? 'GOOD' : 'AVERAGE',
                }
                : null,
        };
            ...
            performance: snapshot
                ? {
                    deliveryTimeRating: getPerformanceRating(
                        ...
                        {
                            excellent: AnalyticsConfig.PERFORMANCE_THRESHOLDS.DRIVER_DELIVERY_TIME_EXCELLENT,
                            good: AnalyticsConfig.PERFORMANCE_THRESHOLDS.DRIVER_DELIVERY_TIME_GOOD,
                        },
     ...
     * Get top products
     */
    async getTopProducts(limit: number = AnalyticsConfig.DEFAULT_LIMITS.TOP_PRODUCTS) {
    ...
    async getOrderFunnelAnalytics(
        ...
        const funnel = events.reduce((acc, curr) => {
