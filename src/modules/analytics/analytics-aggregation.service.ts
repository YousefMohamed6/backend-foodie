...
export class AnalyticsAggregationService {
  ...
  async runWeeklyAggregation() {
    ...
    try {
      ...
      this.logger.error(
        `Weekly aggregation failed: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async runMonthlyAggregation() {
    ...
    try {
      ...
      this.logger.error(
        `Monthly aggregation failed: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async aggregateDriverMetrics(date: Date, type: string) {
    ...
    for (const driver of drivers) {
      try {
        ...
        this.logger.debug(
          `Aggregated ${type} metrics for driver: ${driver.id}`,
        );
      } catch (error) {
        this.logger.error(
  ...
  async aggregateCustomerMetrics(date: Date, type: string) {
    ...
    for (const customer of customers) {
      try {
        ...
        // Get unique vendors
        const vendorCounts = orders.reduce(
          (acc, order) => {
    ...
    for (const zone of zones) {
      try {
        ...
        const cashCollections =
          await this.prisma.managerCashConfirmation.aggregate({
            where: {
              order: {
                vendor: {
                  zoneId: zone.id,
