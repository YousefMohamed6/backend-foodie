...
export class DineInController {
  ...
  async create(@Body() createDineInBookingDto: CreateDineInBookingDto, @Request() req): Promise<DineInBooking> {
  ...
  @ApiQuery({ name: 'isUpcoming', required: false, type: Boolean, description: 'Filter by upcoming (true) or past (false) bookings' })
  async findAll(@Query() query: { isUpcoming?: string | boolean }, @Request() req): Promise<DineInBooking[]> {
