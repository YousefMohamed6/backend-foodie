...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  async catch(exception: any, host: ArgumentsHost) {
    ...
    if (exception instanceof HttpException) {
      ...
      if (status === HttpStatus.BAD_REQUEST && Array.isArray(exceptionResponse.message)) {
        ...
        details = typeof exceptionResponse === 'object' ? exceptionResponse.error : null;
      }
    ...
    if (translatedMessage === messageKey) {
      if (!messageKey.startsWith('messages.')) {
        ...
        translatedMessage = await this.i18n.translate('messages.GENERIC_ERROR', { lang });
      }
    }
  ...
  private mapRawMessageToKey(msg: string): string {
    ...
    const normalized = msg.toUpperCase().replace(/\s+/g, '_').replace(/[^A-Z0-9_]/g, '');
